...
#changing the directory and creating some sub_directories to the created ProjectData directory
cd ProjectData
...
#changing into the sub_directory and creating some text files
cd Data
if [ !  -d Data ];then
...
#intializing some random text to the created text files
...
cd ..
#getting back to the sub directory Reports and creating a file for appending the no.of lines in each file in the above dct Data
cd Reports
...
cd ..
#creating a file in the sub directory Logs for recording the file processing
cd Logs
